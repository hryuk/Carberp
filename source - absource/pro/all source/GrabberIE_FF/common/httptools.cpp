...
bool HttpTools::_parseUrl(const LPSTR url, URLDATA *ud)
{
  ...
  if(InternetCrackUrlA(url, 0, 0, &uc) == FALSE)
  {
    return false;
  }
...
LPWSTR HttpTools::_getFileNameFromUrl(const LPSTR url)
{
  ...
  if(_parseUrl(url, &ud))
  {
    ...
    if(fileName != NULL && fileName[1] != 0)
    {
      ...
      {
        if(UrlUnescapeA(fileName, NULL, NULL, URL_UNESCAPE_INPLACE) == S_OK)
        {
...
bool HttpTools::_compareTagNameA(const LPSTR tag, DWORD tagSize, const LPSTR string)
{
  ...
  if(StrCmpNIA(pos, tag, tagSize) == 0)
  {
    BYTE c = *(pos + tagSize);
...
DWORD HttpTools::_removeTagsA(LPSTR string, DWORD size)
{
  ...
  while(oldOffset < size)
  {
    ...
    {
      if(string[oldOffset] == '&' && (size - oldOffset) > SYMB_SPACE_SIZE &&  StrCmpNIA(string + oldOffset + 1, SYMB_SPACE, SYMB_SPACE_SIZE) == 0)
      {
        string[newOffset] = ' ';
...
LPSTR HttpTools::_getMimeHeader(const void *mimeData, DWORD mimeSize, const LPSTR header, SIZE_T *size)
{
  ...
  *size = 0;
  ...
  while(data < dataEnd)
  {
    ...
    else if(curSize > headerSize && StrCmpNIA(header, cur, headerSize) == 0 && cur[headerSize] == ':')
    {
      dataEnd = cur + curSize;
...
DWORD HttpTools::_removeMimeHeader(void *mimeData, DWORD mimeSize, const LPSTR header)
{
  ...
  while(data < dataEnd)
  {
    ...
    if(curSize > headerSize && StrCmpNIA(header, cur, headerSize) == 0 && cur[headerSize] == ':')
    {
