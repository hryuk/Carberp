...
typedef void WINAPI VNCHOTKEYPROC(HVNC hVNC,int dwID);
bool __cdecl VNCRegisterHotKey(HVNC hVNC,int dwID,UINT dwFsModifiers,UINT dwVK,VNCHOTKEYPROC *lpProc);
bool __cdecl VNCUnregisterHotKey(HVNC hVNC,int dwID);
...
DWORD __cdecl GetVIDevCount();
#define IsCamPresent() GetVIDevCount() > 0
