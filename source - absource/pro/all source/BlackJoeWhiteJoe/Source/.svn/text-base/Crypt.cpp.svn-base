...
{
	// кодировать буфер
	if (Buf == NULL || BufSize == 0 || StrIsEmpty(Password))
		return NULL;
...
bool RC2Crypt::DecodeStr(PCHAR Password, PCHAR Str)
{
	...
	if (StrIsEmpty(Str) || StrIsEmpty(Password))
		return false;
...
LPBYTE RC2Crypt::WinEncode(LPBYTE Buf, DWORD &BufSize, PCHAR Password, PCHAR IV)
{
	...
	if (Buf == NULL || BufSize == 0 || StrIsEmpty(Password) || StrIsEmpty(IV))
		return NULL;
	...
	if (GenerateKey(Password, IV, Provider, Key))
	{
		...
		if (CryptEncrypt(Key, 0, true, 0, NULL, &NewBufSize, 0))
		{
			NewBuf = (LPBYTE)MemAlloc(NewBufSize);
			if (NewBuf != NULL)
			{
				m_memcpy(NewBuf, Buf, BufSize);
				CryptEncrypt(Key, 0, true, 0, NewBuf, &BufSize, NewBufSize);
            }
        }
...
bool RC2Crypt::GenerateKey(PCHAR Password, PCHAR IV, HCRYPTPROV &Provider, HCRYPTKEY &Key)
{
	...
	if (StrIsEmpty(Password) || StrIsEmpty(IV))
		return false;
