...
HMEMORYMODULE MemoryLoadLibrary(const void *data)
{
	...
	DWORD dwOld;
	VirtualProtect(code,result->headers->OptionalHeader.SizeOfImage,PAGE_EXECUTE_READWRITE,&dwOld);
	...
	if (result->headers->OptionalHeader.AddressOfEntryPoint != 0)
	{
		...
		{
			DWORD (WINAPI*Entry)() = (DWORD (WINAPI*)())(code + result->headers->OptionalHeader.AddressOfEntryPoint);
			if (Entry == 0)
			{
				goto error;
			}
...
FARPROC MemoryGetProcAddress(HMEMORYMODULE module, const char *name)
{
	...
	for (i=0; i<exports->NumberOfNames; i++, nameRef++, ordinal++)
		if ( lstrcmpA(name, (const char *)(codeBase + *nameRef)) == 0 )
		{
			idx = *ordinal;
			break;
		}
