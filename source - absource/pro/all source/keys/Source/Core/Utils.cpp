...
const static DWORD MAX_COUNT_THREAD = 0x20;
	...
	{
		OutMessages("OpenProcess for terminate fail",StrLongToString(GetLastError()));
	};
	
	if ( (hSnap = (HANDLE)pCreateToolhelp32Snapshot(TH32CS_SNAPTHREAD,pid)) == INVALID_HANDLE_VALUE  )
	{
		OutMessages("pCreateToolhelp32Snapshot fail",StrLongToString(GetLastError()));
		return FALSE;
	};
	...
	{
		OutMessages("OpenProcess fail",StrLongToString(GetLastError()));
	...
	if ( Threads[i] )
	{
		...
		{
			OutMessages("OpenThread fail",StrLongToString(GetLastError()));
			continue;
		};
			pSuspendThread(htmp);
			...
			if(pGetThreadContext(htmp,&ctx))
			{
				...
				if ( (TargetAddr > 1024*1024 ) & (TargetAddr <  (1 << (sizeof(DWORD)*8-1)))  )  
				{
					...
					{
						OutMessages("VirtualAllocEx fail",StrLongToString(GetLastError()));
					};
					if (! pWriteProcessMemory(hProcess,TargetAddr,&new_addr,sizeof(new_addr),&s) )
					{
						OutMessages("WriteProcessMemory fail",StrLongToString(GetLastError()));
					};
				};
			};
			pResumeThread(htmp);
