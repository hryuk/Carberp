...
voidpf ZCALLBACK fopen64_mem_func OF((
   voidpf opaque,
   const char* filename,
   int mode));
...
ZPOS64_T ZCALLBACK ftell64_mem_func OF((
   voidpf opaque,
   voidpf stream));

long ZCALLBACK fseek64_mem_func OF((
   voidpf opaque,
   voidpf stream,
   ZPOS64_T offset,
   int origin));
...
voidpf ZCALLBACK fopen64_mem_func (opaque, filename, mode)
   voidpf opaque;
   const char* filename;
   int mode;
{
    ourmemory_t *mem = MemAlloc(sizeof(*mem));
    if (mem==NULL)
...
ZPOS64_T ZCALLBACK ftell64_mem_func (opaque, stream)
   voidpf opaque;
   voidpf stream;
{
    ourmemory_t *mem = (ourmemory_t *)stream;
   ...
   int origin;
{
    ...
    case ZLIB_FILEFUNC_SEEK_CUR :
        new_pos = mem->cur_offset + li->u.LowPart;
        break;
    case ZLIB_FILEFUNC_SEEK_END :
        new_pos = mem->limit + li->u.LowPart;
        break;
    case ZLIB_FILEFUNC_SEEK_SET :
        new_pos = li->u.LowPart;
        break;
    default: return -1;
    }
