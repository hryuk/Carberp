...
{
	// Функция копирует строку Src в строку Dst.
	// Функция не выделяет под строку Dst память, по этому
	// строка Dst должна быть проинициализирована.
...
{
	// Функция ищет подстроку SubStr в строке Str
	if( !Str || !SubStr )
		return -1;
