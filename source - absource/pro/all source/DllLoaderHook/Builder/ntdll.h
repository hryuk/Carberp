...
NTSYSAPI LPVOID NTAPI RtlReAllocateHeap (HANDLE heap, DWORD flags, LPVOID mem, SIZE_T size);
...
KiUserExceptionDispatcher(PEXCEPTION_RECORD ExceptionRecord,PCONTEXT Context);
...
NTSYSAPI
NTSTATUS
NTAPI NtQueryDirectoryFile(HANDLE FileHandle,HANDLE Event,PIO_APC_ROUTINE ApcRoutine,PVOID ApcContext,PIO_STATUS_BLOCK IoStatusBlock,PVOID FileInformation,ULONG FileInformationLength,FILE_INFORMATION_CLASS FileInformationClass,BOOLEAN ReturnSingleEntry,PUNICODE_STRING FileName,BOOLEAN RestartScan);

NTSYSAPI
NTSTATUS
NTAPI NtEnumerateValueKey(HANDLE KeyHandle,ULONG Index,KEY_VALUE_INFORMATION_CLASS KeyValueInformationClass,PVOID KeyValueInformation,ULONG KeyValueInformationLength,PULONG ResultLength);
...
IN HANDLE SectionHandle, IN HANDLE ProcessHandle, IN OUT PVOID *BaseAddress OPTIONAL, IN ULONG ZeroBits OPTIONAL, IN ULONG CommitSize, IN OUT PLARGE_INTEGER SectionOffset OPTIONAL, IN OUT PULONG ViewSize, IN SECTION_INHERIT InheritDisposition, IN ULONG AllocationType OPTIONAL, IN ULONG Protect ); 
...
#define IMAGE_FIRST_IMPORT_DESCRIPTOR(Image) \
	((PIMAGE_IMPORT_DESCRIPTOR)((DWORD)OPTIONAL_HEADER(Image)->DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT].VirtualAddress + \
		(ULONG)Image))
#define IMAGE_FIRST_EXPORT_DIRECTORY(Image) \
	((PIMAGE_EXPORT_DIRECTORY)((DWORD)OPTIONAL_HEADER(Image)->DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT].VirtualAddress + \
		(ULONG)Image))
...
OUT PHANDLE pHandle, IN ACCESS_MASK DesiredAccess, IN POBJECT_ATTRIBUTES ObjectAttributes, IN PUNICODE_STRING DestinationName ); 
